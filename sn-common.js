...
const KEY='ndhce_sn_progress_v1';

const TRACKING_MODULE="Special Needs";
...
function resetSNProgress(){
  if(confirm('Reset all Special Needs progress? / 重置所有特殊需求进度？')){
...
function startPractice(bank,title='Practice'){
  ...
  if(!order.length){
    document.getElementById('quiz').innerHTML=`<div class="card center"><h2>No questions found</h2><p>Try another practice option.</p><a class="button primary" href="sn-practice.html">Back to Practice</a></div>`;
    return;
  }
  function load(){
  ...
  function finish(){
    ...
    document.getElementById('quiz').innerHTML=`<div class="card center">
      ...
      <a class="button primary" href="sn-dashboard.html">View dashboard</a>
      <a class="button secondary" href="sn-practice.html">Choose another practice</a>
      <a class="button secondary" href="special-needs.html">Back to hub</a></div>`;
  }
  load();
}
function startExam(bank,minutes=20){
  ...
  function submitExam(){
    ...
    <a class="button primary" href="sn-dashboard.html">View dashboard</a></div>
