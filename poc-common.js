...
const KEY='ndhce_poc_progress_v1';

const TRACKING_MODULE="Process of Care";
...
function resetPOCProgress(){
  if(confirm('Reset all Process of Care progress? / 重置所有指数进度和书签？')){
...
function startPractice(bank,title='Practice'){
  ...
  if(!order.length){
    document.getElementById('quiz').innerHTML=`<div class="card center"><h2>No questions found</h2><p>Try another practice option.</p><a class="button primary" href="poc-practice.html">Back to Practice</a></div>`;
    return;
  }
  function load(){
