...
const KEY='ndhce_law_progress_v1';

const TRACKING_MODULE="Law & Ethics";
...
function resetLawProgress(){
  if(confirm('Reset all Law & Ethics progress? / 重置所有法律与伦理进度？')){
...
function startPractice(bank,title='Practice'){
  ...
  if(!order.length){
    document.getElementById('quiz').innerHTML=`<div class="card center"><h2>No questions found</h2><p>Try another practice option.</p><a class="button primary" href="law-practice.html">Back to Practice</a></div>`;
    return;
  }
  function load(){
  ...
  function finish(){
    ...
    document.getElementById('quiz').innerHTML=`<div class="card center">
      ...
      <a class="button primary" href="law-dashboard.html">View dashboard</a>
      <a class="button secondary" href="law-practice.html">Choose another practice</a>
      <a class="button secondary" href="law-ethics.html">Back to hub</a></div>`;
  }
  load();
}
function startExam(bank,minutes=20){
  ...
  function submitExam(){
    ...
    <a class="button primary" href="law-dashboard.html">View dashboard</a></div>
