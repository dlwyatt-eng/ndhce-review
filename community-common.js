...
const KEY='ndhce_community_progress_v1';

const TRACKING_MODULE="Community Oral Health & Research";
...
function resetCommunityProgress(){
  if(confirm('Reset all Community Oral Health & Research progress? / 重置所有法律与伦理进度？')){
...
function startPractice(bank,title='Practice'){
  ...
  if(!order.length){
    document.getElementById('quiz').innerHTML=`<div class="card center"><h2>No questions found</h2><p>Try another practice option.</p><a class="button primary" href="community-practice.html">Back to Practice</a></div>`;
    return;
  }
  function load(){
...
function startExam(bank,minutes=20){
  ...
  function submitExam(){
    ...
    <a class="button primary" href="community-dashboard.html">View dashboard</a></div>
