...
const KEY='ndhce_med_progress_v2';

const TRACKING_MODULE="Medical Emergencies";
...
function resetMedProgress(){
  if(confirm('Reset all Medical Emergencies progress? / 重置所有指数进度和书签？')){
...
function startPractice(bank,title='Practice'){
  ...
  if(!order.length){
    document.getElementById('quiz').innerHTML=`<div class="card center"><h2>No questions found</h2><p>Try another practice option.</p><a class="button primary" href="med-practice.html">Back to Practice</a></div>`;
    return;
  }
  function load(){
  ...
  function finish(){
    ...
    document.getElementById('quiz').innerHTML=`<div class="card center">
      ...
      <a class="button primary" href="med-dashboard.html">View dashboard</a>
      <a class="button secondary" href="med-practice.html">Choose another practice</a>
      <a class="button secondary" href="medical-emergencies.html">Back to hub</a></div>`;
  }
  load();
}
function startExam(bank,minutes=20){
  ...
  function submitExam(){
    ...
    <a class="button primary" href="med-dashboard.html">View dashboard</a></div>
